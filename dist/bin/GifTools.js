var GifToolsModularized = (function () {
    ...
    return (function (GifToolsModularized) {
        GifToolsModularized = GifToolsModularized || {};
        var Module = typeof GifToolsModularized !== "undefined" ? GifToolsModularized : {};
        var moduleOverrides = {};
        var key;
        for (key in Module) {
        ...
        var wasmMemory;
        var wasmTable = new WebAssembly.Table({ "initial": 88, "maximum": 88 + 0, "element": "anyfunc" });
        var ABORT = false;
        var EXITSTATUS = 0;
        ...
        function UTF16ToString(ptr) { var endPtr = ptr; var idx = endPtr >> 1; while (HEAP16[idx])
        ...
        function UTF32ToString(ptr) { var i = 0; var str = ""; while (1) {
        ...
        var DYNAMIC_BASE = 5259840, DYNAMICTOP_PTR = 16800;
        ...
        var wasmBinaryFile = "GifTools.wasm";
        ...
        function ___cxa_throw(ptr, type, destructor) { ___exception_infos[ptr] = { ptr: ptr, adjusted: [ptr], type: type, destructor: destructor, refcount: 0, caught: false, rethrown: false }; ___exception_last = ptr; if (!("uncaught_exception" in __ZSt18uncaught_exceptionv)) {
        ...
        var MEMFS = { ops_table: null, mount: function (mount) { return MEMFS.createNode(null, "/", 16384 | 511, 0); }, createNode: function (parent, name, mode, dev) { if (FS.isBlkdev(mode) || FS.isFIFO(mode)) {
                ...
                while (node.contents.length < newSize)
                    ...
                    throw new FS.ErrnoError(43);
                } var ptr; var allocated; var contents = stream.node.contents; if (!(flags & 2) && contents.buffer === buffer.buffer) {
                    allocated = false;
        ...
        var FS = { root: null, mounts: [], devices: {}, streams: [], nextInode: 1, nameTable: null, currentPath: "/", initialized: false, ignorePermissions: true, trackingDelegate: {}, tracking: { openFlags: { READ: 1, WRITE: 2 } }, ErrnoError: null, genericErrors: {}, filesystems: null, syncFSRequests: 0, handleFSError: function (e) { if (!(e instanceof FS.ErrnoError))
                throw e + " : " + stackTrace(); return ___setErrNo(e.errno); }, lookupPath: function (path, opts) { path = PATH_FS.resolve(FS.cwd(), path); opts = opts || {}; if (!path)
            ...
            } stream.stream_ops.allocate(stream, offset, length); }, mmap: function (stream, buffer, offset, length, position, prot, flags) { if ((prot & 2) !== 0 && (flags & 2) === 0 && (stream.flags & 2097155) !== 2) {
                ...
                path = path.substr(1); return path; }, absolutePath: function (relative, base) { return PATH_FS.resolve(base, relative); }, standardizePath: function (path) { return PATH.normalize(path); }, findObject: function (path, dontResolveLastLink) { var ret = FS.analyzePath(path, dontResolveLastLink); if (ret.exists) {
                return ret.object;
            }
            else {
                ...
                ___setErrNo(29); return success; }, createLazyFile: function (parent, name, url, canRead, canWrite) { function LazyUint8Array() { this.lengthKnown = false; this.chunks = []; } LazyUint8Array.prototype.get = function LazyUint8Array_get(idx) { if (idx > this.length - 1 || idx < 0) {
                return undefined;
        ...
        function ___syscall221(fd, cmd, varargs) { SYSCALLS.varargs = varargs; try {
        ...
        function ___syscall5(path, flags, varargs) { SYSCALLS.varargs = varargs; try {
        ...
        function ___syscall54(fd, op, varargs) { SYSCALLS.varargs = varargs; try {
            var stream = SYSCALLS.getStreamFromFD(fd);
            switch (op) {
                case 21509:
                case 21505: {
        ...
        function embind__requireFunction(signature, rawFunction) { signature = readLatin1String(signature); function makeDynCaller(dynCall) { var args = []; for (var i = 1; i < signature.length; ++i) {
            args.push("a" + i);
        } var name = "dynCall_" + signature + "_" + rawFunction; var body = "return function " + name + "(" + args.join(", ") + ") {\n"; body += "    return dynCall(rawFunction" + (args.length ? ", " : "") + args.join(", ") + ");\n"; body += "};\n"; return new Function("dynCall", "rawFunction", body)(dynCall, rawFunction); } var dc = Module["dynCall_" + signature]; var fp = makeDynCaller(dc); if (typeof fp !== "function") {
        ...
        function __embind_register_std_wstring(rawType, charSize, name) { name = readLatin1String(name); var decodeString, encodeString, getHeap, lengthBytesUTF, shift; if (charSize === 2) {
            ...
            shift = 2;
        } registerType(rawType, { name: name, "fromWireType": function (value) { var length = HEAPU32[value >> 2]; var HEAP = getHeap(); var str; var endChar = HEAP[value + 4 + length * charSize >> shift]; var endCharSwap = 0; if (endChar != 0) {
                endCharSwap = endChar;
        ...
        function _emscripten_resize_heap(requestedSize) { var oldSize = _emscripten_get_heap_size(); var PAGE_MULTIPLE = 65536; var maxHeapSize = 2147483648 - PAGE_MULTIPLE; if (requestedSize > maxHeapSize) {
            return false;
        ...
        var asmLibraryArg = { "a": ___assert_fail, "g": ___cxa_allocate_exception, "f": ___cxa_throw, "q": ___syscall221, "C": ___syscall5, "B": ___syscall54, "y": __embind_register_bool, "x": __embind_register_emval, "m": __embind_register_float, "r": __embind_register_function, "c": __embind_register_integer, "b": __embind_register_memory_view, "n": __embind_register_std_string, "e": __embind_register_std_wstring, "z": __embind_register_void, "j": __emval_as, "d": __emval_decref, "k": __emval_get_property, "l": __emval_incref, "s": __emval_new_cstring, "i": __emval_run_destructors, "h": __emval_take_value, "v": _emscripten_memcpy_big, "w": _emscripten_resize_heap, "o": _fd_close, "A": _fd_read, "t": _fd_seek, "p": _fd_write, "memory": wasmMemory, "u": _setTempRet0, "table": wasmTable };
        var asm = createWasm();
        Module["asm"] = asm;
        ...
        var _testPing = Module["_testPing"] = function () { return (_testPing = Module["_testPing"] = Module["asm"]["G"]).apply(null, arguments); };
        var _testAdd = Module["_testAdd"] = function () { return (_testAdd = Module["_testAdd"] = Module["asm"]["H"]).apply(null, arguments); };
        var _testDump = Module["_testDump"] = function () { return (_testDump = Module["_testDump"] = Module["asm"]["I"]).apply(null, arguments); };
        var _objectFree = Module["_objectFree"] = function () { return (_objectFree = Module["_objectFree"] = Module["asm"]["J"]).apply(null, arguments); };
        var _bufferCopyFromMemory = Module["_bufferCopyFromMemory"] = function () { return (_bufferCopyFromMemory = Module["_bufferCopyFromMemory"] = Module["asm"]["K"]).apply(null, arguments); };
        var _bufferMutableData = Module["_bufferMutableData"] = function () { return (_bufferMutableData = Module["_bufferMutableData"] = Module["asm"]["L"]).apply(null, arguments); };
        var _bufferData = Module["_bufferData"] = function () { return (_bufferData = Module["_bufferData"] = Module["asm"]["M"]).apply(null, arguments); };
        var _bufferSize = Module["_bufferSize"] = function () { return (_bufferSize = Module["_bufferSize"] = Module["asm"]["N"]).apply(null, arguments); };
        var _bufferResize = Module["_bufferResize"] = function () { return (_bufferResize = Module["_bufferResize"] = Module["asm"]["O"]).apply(null, arguments); };
        var _bufferReserve = Module["_bufferReserve"] = function () { return (_bufferReserve = Module["_bufferReserve"] = Module["asm"]["P"]).apply(null, arguments); };
        var _bufferFree = Module["_bufferFree"] = function () { return (_bufferFree = Module["_bufferFree"] = Module["asm"]["Q"]).apply(null, arguments); };
        var _bufferZeroTerminated = Module["_bufferZeroTerminated"] = function () { return (_bufferZeroTerminated = Module["_bufferZeroTerminated"] = Module["asm"]["R"]).apply(null, arguments); };
        var _bufferEmpty = Module["_bufferEmpty"] = function () { return (_bufferEmpty = Module["_bufferEmpty"] = Module["asm"]["S"]).apply(null, arguments); };
        var _bufferToStringBase64 = Module["_bufferToStringBase64"] = function () { return (_bufferToStringBase64 = Module["_bufferToStringBase64"] = Module["asm"]["T"]).apply(null, arguments); };
        var _bufferFromStringBase64 = Module["_bufferFromStringBase64"] = function () { return (_bufferFromStringBase64 = Module["_bufferFromStringBase64"] = Module["asm"]["U"]).apply(null, arguments); };
        var _pixelFormatByteWidth = Module["_pixelFormatByteWidth"] = function () { return (_pixelFormatByteWidth = Module["_pixelFormatByteWidth"] = Module["asm"]["V"]).apply(null, arguments); };
        var _imageWidth = Module["_imageWidth"] = function () { return (_imageWidth = Module["_imageWidth"] = Module["asm"]["W"]).apply(null, arguments); };
        var _imageHeight = Module["_imageHeight"] = function () { return (_imageHeight = Module["_imageHeight"] = Module["asm"]["X"]).apply(null, arguments); };
        var _imageFormat = Module["_imageFormat"] = function () { return (_imageFormat = Module["_imageFormat"] = Module["asm"]["Y"]).apply(null, arguments); };
        var _imageClone = Module["_imageClone"] = function () { return (_imageClone = Module["_imageClone"] = Module["asm"]["Z"]).apply(null, arguments); };
        var _imageLoadFromMemory = Module["_imageLoadFromMemory"] = function () { return (_imageLoadFromMemory = Module["_imageLoadFromMemory"] = Module["asm"]["_"]).apply(null, arguments); };
        var _imageLoadFromBuffer = Module["_imageLoadFromBuffer"] = function () { return (_imageLoadFromBuffer = Module["_imageLoadFromBuffer"] = Module["asm"]["$"]).apply(null, arguments); };
        var _imageResizeOrClone = Module["_imageResizeOrClone"] = function () { return (_imageResizeOrClone = Module["_imageResizeOrClone"] = Module["asm"]["aa"]).apply(null, arguments); };
        var _imageExportToPNG = Module["_imageExportToPNG"] = function () { return (_imageExportToPNG = Module["_imageExportToPNG"] = Module["asm"]["ba"]).apply(null, arguments); };
        var _gifBuilderInitialize = Module["_gifBuilderInitialize"] = function () { return (_gifBuilderInitialize = Module["_gifBuilderInitialize"] = Module["asm"]["ca"]).apply(null, arguments); };
        var _gifBuilderAddImage = Module["_gifBuilderAddImage"] = function () { return (_gifBuilderAddImage = Module["_gifBuilderAddImage"] = Module["asm"]["da"]).apply(null, arguments); };
        var _gifBuilderFinalize = Module["_gifBuilderFinalize"] = function () { return (_gifBuilderFinalize = Module["_gifBuilderFinalize"] = Module["asm"]["ea"]).apply(null, arguments); };
        ...
        var ___getTypeName = Module["___getTypeName"] = function () { return (___getTypeName = Module["___getTypeName"] = Module["asm"]["ga"]).apply(null, arguments); };
        ...
        var stackAlloc = Module["stackAlloc"] = function () { return (stackAlloc = Module["stackAlloc"] = Module["asm"]["ia"]).apply(null, arguments); };
        var dynCall_ii = Module["dynCall_ii"] = function () { return (dynCall_ii = Module["dynCall_ii"] = Module["asm"]["ja"]).apply(null, arguments); };
        var dynCall_vi = Module["dynCall_vi"] = function () { return (dynCall_vi = Module["dynCall_vi"] = Module["asm"]["ka"]).apply(null, arguments); };
        var dynCall_iiiiii = Module["dynCall_iiiiii"] = function () { return (dynCall_iiiiii = Module["dynCall_iiiiii"] = Module["asm"]["la"]).apply(null, arguments); };
        ...
        var dynCall_viii = Module["dynCall_viii"] = function () { return (dynCall_viii = Module["dynCall_viii"] = Module["asm"]["na"]).apply(null, arguments); };
        var dynCall_iiii = Module["dynCall_iiii"] = function () { return (dynCall_iiii = Module["dynCall_iiii"] = Module["asm"]["oa"]).apply(null, arguments); };
        var dynCall_iiiii = Module["dynCall_iiiii"] = function () { return (dynCall_iiiii = Module["dynCall_iiiii"] = Module["asm"]["pa"]).apply(null, arguments); };
        var dynCall_iii = Module["dynCall_iii"] = function () { return (dynCall_iii = Module["dynCall_iii"] = Module["asm"]["qa"]).apply(null, arguments); };
        var dynCall_ff = Module["dynCall_ff"] = function () { return (dynCall_ff = Module["dynCall_ff"] = Module["asm"]["ra"]).apply(null, arguments); };
        var dynCall_fff = Module["dynCall_fff"] = function () { return (dynCall_fff = Module["dynCall_fff"] = Module["asm"]["sa"]).apply(null, arguments); };
        var dynCall_vii = Module["dynCall_vii"] = function () { return (dynCall_vii = Module["dynCall_vii"] = Module["asm"]["ta"]).apply(null, arguments); };
        var dynCall_jiji = Module["dynCall_jiji"] = function () { return (dynCall_jiji = Module["dynCall_jiji"] = Module["asm"]["ua"]).apply(null, arguments); };
        var dynCall_viiiii = Module["dynCall_viiiii"] = function () { return (dynCall_viiiii = Module["dynCall_viiiii"] = Module["asm"]["va"]).apply(null, arguments); };
        ...
        Module["asm"] = asm;
        var calledRun;
        Module["then"] = function (func) { if (calledRun) {
            func(Module);
        }
        else {
        ...
        noExitRuntime = true;
        run();
        return GifToolsModularized;
    });
})();
if (typeof exports === 'object' && typeof module === 'object')
    module.exports = GifToolsModularized;
else if (typeof define === 'function' && define['amd'])
    define([], function () { return GifToolsModularized; });
else if (typeof exports === 'object')
    exports["GifToolsModularized"] = GifToolsModularized;
//# sourceMappingURL=GifTools.js.map