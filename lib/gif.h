...
struct GifVectorFileBuffer : GifFileBuffer {
    static int Close(void *stream) { return 0; }
    ...
    static size_t Write(const void*  p, size_t sz, size_t nitems, void* stream) {
        GifVectorFileBuffer* buffer = (GifVectorFileBuffer*)stream; assert(buffer);
        const size_t contentsFilled = buffer->contents.size();
    ...
    GifVectorFileBuffer() {
        ...
        this->open = &GifVectorFileBuffer::Open;
        this->close = &GifVectorFileBuffer::Close;
        this->putc = &GifVectorFileBuffer::Putc;
        this->puts = &GifVectorFileBuffer::Puts;
        this->write = &GifVectorFileBuffer::Write;
    }
};
...
struct GifStdioFileBuffer : GifFileBuffer {
    ...
    GifStdioFileBuffer() {
        this->open = &GifStdioFileBuffer::Open;
        this->close = &GifStdioFileBuffer::Close;
        this->putc = &GifStdioFileBuffer::Putc;
        this->puts = &GifStdioFileBuffer::Puts;
        this->write = &GifStdioFileBuffer::Write;
    }
};
...
void GifWriteChunk( GifFileBuffer* buffer, GifBitStatus& stat )
{
    buffer->putc((int)stat.chunkIndex, buffer->stream);
    buffer->write(stat.chunk, 1, stat.chunkIndex, buffer->stream);
...
void GifWriteCode( GifFileBuffer* buffer, GifBitStatus& stat, uint32_t code, uint32_t length )
{
...
void GifWritePalette( const GifPalette* pPal, GifFileBuffer* buffer )
{
    buffer->putc(0, buffer->stream);  // first color: transparency
    buffer->putc(0, buffer->stream);
    buffer->putc(0, buffer->stream);
    ...
    for(int ii=1; ii<(1 << pPal->bitDepth); ++ii)
    {
        ...
        buffer->putc((int)r, buffer->stream);
        buffer->putc((int)g, buffer->stream);
        buffer->putc((int)b, buffer->stream);
    }
}

// write the image header, LZW-compress and write out the image
void GifWriteLzwImage(GifFileBuffer* buffer, uint8_t* image, uint32_t left, uint32_t top,  uint32_t width, uint32_t height, uint32_t delay, GifPalette* pPal)
{
    // graphics control extension
    buffer->putc(0x21, buffer->stream);
    buffer->putc(0xf9, buffer->stream);
    buffer->putc(0x04, buffer->stream);
    buffer->putc(0x05, buffer->stream); // leave prev frame in place, this frame has transparency
    buffer->putc(delay & 0xff, buffer->stream);
    buffer->putc((delay >> 8) & 0xff, buffer->stream);
    buffer->putc(kGifTransIndex, buffer->stream); // transparent color index
    buffer->putc(0, buffer->stream);

    buffer->putc(0x2c, buffer->stream); // image descriptor block

    buffer->putc(left & 0xff, buffer->stream);           // corner of image in canvas space
    buffer->putc((left >> 8) & 0xff, buffer->stream);
    buffer->putc(top & 0xff, buffer->stream);
    buffer->putc((top >> 8) & 0xff, buffer->stream);

    buffer->putc(width & 0xff, buffer->stream);          // width and height of image
    buffer->putc((width >> 8) & 0xff, buffer->stream);
    buffer->putc(height & 0xff, buffer->stream);
    buffer->putc((height >> 8) & 0xff, buffer->stream);

    //fputc(0, f); // no local color table, no transparency
    //fputc(0x80, f); // no local color table, but transparency

    buffer->putc(0x80 + pPal->bitDepth-1, buffer->stream); // local color table present, 2 ^ bitDepth entries
    GifWritePalette(pPal, buffer);

    const int minCodeSize = pPal->bitDepth;
    const uint32_t clearCode = 1 << pPal->bitDepth;

    buffer->putc(minCodeSize, buffer->stream); // min code size 8 bits
    ...
    for(uint32_t yy=0; yy<height; ++yy)
    {
        for(uint32_t xx=0; xx<width; ++xx)
        {
            ...
            else
            {
                ...
                GifWriteCode(buffer, stat, (uint32_t)curCode, codeSize);
    ...
    GifWriteCode(buffer, stat, clearCode + 1, (uint32_t)minCodeSize + 1);

    // write out the last partial chunk
    while( stat.bitIndex ) GifWriteBit(stat, 0);
    if( stat.chunkIndex ) GifWriteChunk(buffer, stat);

    buffer->putc(0, buffer->stream); // image block terminator
